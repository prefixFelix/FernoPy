...
counter = 1
tx = machine.Pin(config.general['tx_pin'], machine.Pin.OUT)
...
def connect():
    ...
    if 'static_ip' in config.general and config.general['static_ip']['enabled']:
        sta_if.ifconfig((
            config.general['static_ip']['ip'],
            config.general['static_ip']['subnet_mask'],
            config.general['static_ip']['gateway'],
            config.general['static_ip']['dns']
        ))

    sta_if.connect(config.general['essid'], config.general['password'])
    print(f'\n\n\n[>] Connecting to {config.general["essid"]}', end='')
...
async def get_config(request):
    response = {'remotes': [], 'style': config.general['style']}
    for remotes in config.fernotron:
...
async def command(request):
    ...
    device_type = config.fernotron[remote_num]['device_type']
    device_id = config.fernotron[remote_num]['device_id']
    ...
    fernotron.send_msg(msg_bits, config.general['tx_repeat'], tx)
...
async def index(request):
    ...
    await send_file(
        request,
        f'./{config.general["html_assets"]}/index.html',
    )
